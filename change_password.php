...
// For storing errors:
$pass_errors = array();
// If it's a POST request, handle the form submission conditional area:
...
$q = "SELECT uid FROM users WHERE pass='" . get_password_hash($current) . "' AND uid={$_SESSION['user_id']}";
$r = mysqli_query ($dbc, $q);
if (mysqli_num_rows($r) == 1) { // Correct
// Define the query:
$q = "UPDATE users SET pass='" .get_password_hash($p)."' WHERE uid={$_SESSION['user_id']} LIMIT 1";
