...
// This script is created in Ecommerce Lab 1.
...
$q = 'SELECT uid FROM users WHERE email="'. mysqli_real_escape_string ($dbc, $_POST['email']) . '"';
...
// Update the database:
$q = "UPDATE users SET pass='" . get_password_hash($p) . "' WHERE uid=$uid LIMIT 1";
$r = mysqli_query ($dbc, $q);
if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
// Send an email:
$message = "Your password to log into MAS.org (Missouri Academy of Science) has been temporarily changed to '$p'. Please log in using that password and this email address. Then you may change your password to something more familiar.";
mail ($_POST['email'], 'Your temporary password.', $message, 'From: mas@csmwsu.org');
